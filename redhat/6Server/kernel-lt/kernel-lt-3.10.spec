...
Provides: kernel-lt = %{version}-%{release}
Provides: kernel-lt-%{_target_cpu} = %{version}-%{release}
Provides: kernel-lt-drm = 4.3.0
Provides: kernel-lt-drm-nouveau = 16
Provides: kernel-lt-modeset = 1
Provides: kernel-lt-uname-r = %{version}-%{release}.%{_target_cpu}
...
Provides: kernel-lt-devel = %{version}-%{release}
Provides: kernel-lt-devel-uname-r = %{version}-%{release}.%{_target_cpu}
...
Provides: kernel-uname-r = %{version}-%{release}.%{_target_cpu}
Provides: kernel-lt = %{version}-%{release}
Provides: kernel-lt-%{_target_cpu} = %{version}-%{release}NONPAE
Provides: kernel-lt-NONPAE = %{version}-%{release}
Provides: kernel-lt-NONPAE-%{_target_cpu} = %{version}-%{release}NONPAE
Provides: kernel-lt-drm = 4.3.0
Provides: kernel-lt-drm-nouveau = 16
Provides: kernel-lt-modeset = 1
Provides: kernel-lt-uname-r = %{version}-%{release}.%{_target_cpu}
...
Provides: kernel-lt-NONPAE-devel-%{_target_cpu} = %{version}-%{release}
Provides: kernel-lt-NONPAE-devel = %{version}-%{release}NONPAE
Provides: kernel-lt-NONPAE-devel-uname-r = %{version}-%{release}.%{_target_cpu}
Requires(pre): /usr/bin/find
AutoReqProv: no
...
hwcap 1 nosegneg"
    ...
    if [ ! -s ldconfig-kernel-lt.conf ]; then
      echo > ldconfig-kernel-lt.conf "\
# Placeholder file, no vDSO hwcap entries used in this kernel."
    fi
    %{__install} -D -m 444 ldconfig-kernel-lt.conf $RPM_BUILD_ROOT/etc/ld.so.conf.d/kernel-lt-%{KVRFA}.conf
%endif
...
%if %{with_dracut}
/sbin/new-kernel-pkg --package kernel-lt --mkinitrd --dracut --depmod --update %{version}-%{release}.%{_target_cpu} $NEWKERNARGS || exit $?
%else
/sbin/new-kernel-pkg --package kernel-lt --mkinitrd --depmod --update %{version}-%{release}.%{_target_cpu} $NEWKERNARGS || exit $?
%endif
/sbin/new-kernel-pkg --package kernel-lt --rpmposttrans %{version}-%{release}.%{_target_cpu} || exit $?
...
/sbin/new-kernel-pkg --package kernel-lt --install %{version}-%{release}.%{_target_cpu} || exit $?
...
%if %{with_dracut}
/sbin/new-kernel-pkg --package kernel-lt-NONPAE --mkinitrd --dracut --depmod --update %{version}-%{release}NONPAE.%{_target_cpu} $NEWKERNARGS || exit $?
%else
/sbin/new-kernel-pkg --package kernel-lt-NONPAE --mkinitrd --depmod --update %{version}-%{release}NONPAE.%{_target_cpu} $NEWKERNARGS || exit $?
%endif
/sbin/new-kernel-pkg --package kernel-lt-NONPAE --rpmposttrans %{version}-%{release}NONPAE.%{_target_cpu} || exit $?
if [ -x /sbin/weak-modules ]; then
...
/sbin/new-kernel-pkg --package kernel-lt-NONPAE --install %{version}-%{release}NONPAE.%{_target_cpu} || exit $?
...
/etc/ld.so.conf.d/kernel-lt-%{version}-%{release}.%{_target_cpu}.conf
%endif
...
/etc/ld.so.conf.d/kernel-lt-%{version}-%{release}NONPAE.%{_target_cpu}.conf
%endif
