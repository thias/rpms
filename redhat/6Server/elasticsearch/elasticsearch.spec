# Taken from https://github.com/tavisto/elasticsearch-rpms
...
Version:        0.19.11
Release:        1%{?dist}
Summary:        Distributed, highly available, RESTful search engine
...
%description
Distributed, highly available, RESTful search engine.

%prep
%setup -q

%build
# Java, nothing to see here...
...
%{__install} -m 644 %{SOURCE4} %{buildroot}%{_sysconfdir}/sysconfig/elasticsearch

%{__mkdir} -p %{buildroot}%{_localstatedir}/run/elasticsearch

%pre
# create elastics group
if ! getent group elastics &>/dev/null; then
    groupadd -r elastics
fi

# create elastics user
if ! getent passwd elastics &>/dev/null; then
    useradd -r -g elastics -d %{_javadir}/%{name} \
        -s /sbin/nologin -c "You know, for search" elastics
fi
...
%preun
if [ $1 -eq 0 ]; then
    /sbin/service elasticsearch stop &>/dev/null
...
%changelog
* Tue Nov 13 2012 Matthias Saou <matthias@saou.eu> 0.19.11-1
- Update to 0.19.11.
- Rename the user "elastics" to stay at 8 chars for convenience.
